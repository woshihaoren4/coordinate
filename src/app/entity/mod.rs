...
pub async fn lock<Out,Handle>(lock:Arc<dyn GlobalLock>,lock_key:String,value:String,timeout:Duration,handle:Handle)->anyhow::Result<Out>
    where Handle: Future<Output=anyhow::Result<Out>> + Send
{
    //加锁
    let result = lock.lock(lock_key.clone(),value.clone(),timeout).await?;
    if !result{
...
impl EntityStore for db::EtcdClient {
    ...
    async fn register_nodes(&self, task_id: i64, node: &NodeEntity) -> anyhow::Result<()> {
        let key = format!("{}/node/{}/{}",common::DB_VERSION,task_id,node.code.as_str());
    ...
    async fn get_slot_detail(&self, task_id: i64) -> anyhow::Result<Vec<NodeEntity>> {
        ...
        let mut range = self.client.get(key).await?;
        if range.kvs.len() > 0 {
            let ns = serde_json::from_slice::<Vec<NodeEntity>>(range.kvs.remove(0).value.as_slice())?;
            Ok(ns)
        }else{
            Ok(Vec::new())
        }
