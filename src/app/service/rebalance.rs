...
impl SoltRebalance<'_> {
    ...
    pub fn balance(self) -> Vec<NodeEntity>{
        ...
        let max = if slot.node_max_count > exp{exp}else{slot.node_max_count};
        ...
        for i in list.iter_mut(){
            if i.tags.len() as i32 > max {    //多于应该则删除
                let index = if more > 0 {
                    more -= 1;
                    max
                }else{
...
#[cfg(test)]
mod test{
    use crate::app::entity::{TaskEntity, TaskSlot, NodeEntity};
    ...
    fn test_join_four(){
        ...
        let result = SoltRebalance::new(&slot, vec![node_one,node_two,node_four]).join(node_three);
        println!("{:?}",result)
        //17, 9, 4, 21, 8, 16, 31, 28, 25, 23, 30
        //15, 6, 13, 3, 5, 20, 7, 0, 2, 29, 19
        //26, 10, 1, 11, 24, 14, 18, 12, 27, 22
